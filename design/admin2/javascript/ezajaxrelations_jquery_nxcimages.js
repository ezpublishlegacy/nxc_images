jQuery(function( $ )
{
    ...
    function _search( e )
    {
        ...
        if ( text.val() )
        {
            ...
            $.ez( 'nxcimages::search::' + text.val(), params, _searchCallBack );
        }
        return false;
    }
        ...
        if ( data && data.content !== '' )
        {
            ...
            if ( data.content.SearchResultCount )
            {
                ...
                for ( var i = 0, l = arr.length; i < l; i++ )
                {
                    ...
                    html += '<a onclick="return ezajaxrelationsSearchAddObject( this, \'' + boxID + '\', ' + arr[i].id + ',\'' + arr[i].name + '\',\'' + arr[i].class_name + '\',\'' + arr[i].section.name + '\',\'' + pub.val() + '\'  );" title="' + arr[i].path_identification_string + '">' + image + arr[i].name + '<\/a>';
                }
                $( boxID + ' div.ezobject-relation-search-browse'  ).html( '<div class="m-search-results">' + html + '</div>' ).show();
            }
            else
            {
