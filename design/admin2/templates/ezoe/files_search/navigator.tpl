...
<div class="pagenavigator">
	<p>
		...
		{if gt( $current_page, 1 )}
			<span class="previous"><a href="{concat( 'nxc_files/search/', $parent_node_id, '/', $facet_node_id, '/', sub( $current_page, 2 )|mul( $limit ), '/', $limit, '?q=', $search_word )|ezurl( 'no' )}"><span class="text">&laquo;&nbsp;{'Previous'|i18n( 'design/admin/navigator' )}</span></a></span>
		{else}
		...
		{for 0 to $pages_count as $page}
			{if or(
				...
				{else}
					<span class="other"><a href="{concat( 'nxc_files/search/', $parent_node_id, '/', $facet_node_id, '/', $page|mul( $limit ), '/', $limit, '?q=', $search_word )|ezurl( 'no' )}">{sum( $page, 1 )}</a></span>
				{/if}
			{else}
				{if and(
					eq( $left, false() ),
	    ...
	    {if lt( $current_page, $pages_count )}
			<span class="next"><a href="{concat( 'nxc_files/search/', $parent_node_id, '/', $facet_node_id, '/', $current_page|mul( $limit ), '/', $limit, '?q=', $search_word )|ezurl( 'no' )}"><span class="text">{'Next'|i18n( 'design/admin/navigator' )}&nbsp;&raquo;</span></a></span>
		{else}
